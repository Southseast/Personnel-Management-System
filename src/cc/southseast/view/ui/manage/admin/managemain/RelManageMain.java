...
import cc.southseast.controller.function.batchdelete.ToBatchDeleteRel;
import cc.southseast.controller.function.search.ToSearchRel;
 ...
 * 职工部门关系管理界面
 */
public class RelManageMain extends VBox {
    ...
    private RelTableView relTableView = new RelTableView();

    public RelManageMain() {
        ...
        searchButton.setOnAction(new ToSearchRel(searchInput, relTableView));

        batchDeletionButton.setId(BATCH_DELETE_BUTTON_CSS_ID);
        batchDeletionButton.setOnAction(new ToBatchDeleteRel(relTableView));

        functionBar.setSpacing(20);
        functionBar.getChildren().addAll(searchInput, searchButton, batchDeletionButton);

        this.getChildren().addAll(functionBar, relTableView);
    }
}